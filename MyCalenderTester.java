...
public class MyCalenderTester {
	...
	public static void main(String [] args){
		...
		LocalDate eventEndDay = LocalDate.now();
		LocalTime eventTime = LocalTime.now();
		LocalTime eventEndTime = LocalTime.now();
		...
		try {
			...
			while (eventScanner.hasNext()) {
				...
				if (eventDetailsArray.length == 3) {
					...
					year = 2000 + detailScanner.nextInt();
					detailScanner.close();
					eventEndDay = LocalDate.of(year, month, endDay);
					int endDate = eventEndDay.getDayOfYear();
					...
					endMins = detailScanner.nextInt();
					detailScanner.close();
					eventEndTime = LocalTime.of(endHour, endMins);

					
					int numDays = daysInWeek.length;
					int dayDiff = 0;
					for(int j = 0; j < daysInWeek.length; j++) {
						if (j==0) {
							startDay = eventDay.getDayOfYear();
							for(int z = startDay; z <= endDate;) {
								eventDay = eventDay.ofYearDay(year, z);
								eventDateTime = LocalDateTime.of(eventDay, eventTime);
								calenderEvent.addBegDateTime(eventDateTime);
								eventDateTime = LocalDateTime.of(eventDay, eventEndTime);
								calenderEvent.addEndDateTime(eventDateTime);
								z += 7;
							}
						} 
